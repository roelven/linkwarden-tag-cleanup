#!/bin/bash
# Wrapper script for normalize_new_tags.py
...
# Run normalization
python3 "$PROJECT_ROOT/scripts/normalize_new_tags.py" \
    --api-url "$LINKWARDEN_API_URL" \
    --token "$LINKWARDEN_TOKEN" \
    --lookback "${LOOKBACK_MINUTES:-15}" \
    --similarity-threshold "${SIMILARITY_THRESHOLD:-0.85}" \
    "$@"
